...
The review stage runs AFTER the emit-code stage. This means review can inspect
generated code files on disk in addition to the AGENTS.md and analysis outputs.

Review considerations for emitted code:
- Validate that emitted files exist at paths declared in `manifest.json`
- Check that emitted exports match the manifest's `files[].exports[]`
- Flag any emitted code that conflicts with existing project files (overwrites)
- Review emitted code for correctness, safety, and style consistency

### Envelope

The review stage produces the standard envelope alongside a
`<repo-root>/.harness-kit/review.json`.
